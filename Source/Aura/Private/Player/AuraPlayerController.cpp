...
void AAuraPlayerController::AutoRun()
{
	...
	{
		const FVector LocOnSpline = Spline->FindLocationClosestToWorldLocation(ControllerPawn->GetActorLocation(), ESplineCoordinateSpace::World);
		const FVector Direction = Spline->FindDirectionClosestToWorldLocation(LocOnSpline, ESplineCoordinateSpace::World);
		ControllerPawn->AddMovementInput(Direction);
		const float DistToDestination = (LocOnSpline - CachedDestination).Length();
...
{
	GetHitResultUnderCursor(ECC_Navigation, false, CursorHit);
